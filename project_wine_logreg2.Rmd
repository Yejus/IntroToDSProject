...
whitewine_data <- read.csv("~/Downloads/winequality-white.csv", sep=";", stringsAsFactors=FALSE)
...
Decision tree full-red wine
```{r}
library("rpart")
library("rpart.plot")

fit  <- rpart(qual_above_med ~ fixed.acidity + volatile.acidity + citric.acid + residual.sugar + chlorides + free.sulfur.dioxide + total.sulfur.dioxide + density + pH + sulphates + alcohol, data=redwine_data,control=rpart.control(minsplit=80), method="class",
...
Decision tree-full white wine
```{r}
fit2  <- rpart( qual_above_med ~ fixed.acidity + volatile.acidity + citric.acid + residual.sugar + chlorides + free.sulfur.dioxide + total.sulfur.dioxide + density + pH + sulphates + alcohol,
method="class",
data=whitewine_data, control=rpart.control(minsplit=1),
parms=list(split='information'))
...
Decision tree-red wine: remooced predictors
```{r}
