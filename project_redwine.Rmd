...
Intended Approach:  
We can use logistic regression or decision trees to construct the prediction model.  

**Research Question 2:**  
By looking at the 12 input variables in the data set which represent the concentration of the chemical compounds, are we able to predict the quality of the wine?  
This research question will be explored by three different subsets of data: 1) red wines only; 2) white wines only; and 3) both red and white wines. 

Intended Approach:  
We can split the possible values of the quality of wine (0 - 10) into bands where each range will represent a class label. For example, values between 0-5 could represent 'bad' wine, while values between 6-10 could represent 'good' wine. We will then use logistic regression or decision trees to construct the prediction model.   

**Research Question 3:**  
Are there any underlying patterns within the dataset that are not obvious?  

Intended Approach:  
We can first perform variable selection using stepwise regression, after which we can perform k-means clustering and interpret the results.  
