...
whitewine_data <- read.csv("winequality-white.csv")
...
Redwine with all predictors
```{r}
library("rpart")
library("rpart.plot")
fit  <- rpart(qual_above_med ~ fixed.acidity + volatile.acidity + citric.acid + residual.sugar + chlorides + free.sulfur.dioxide + total.sulfur.dioxide + density + pH + sulphates + alcohol, data=redwine_data,control=rpart.control(minsplit=160), method="class",
...
```{r}
pred <- prediction(predict(fit, type = "prob")[, 2], redwine_data$qual_above_med)
...
Redwine with correlated varaibles removed


```{r}
fit2  <- rpart(qual_above_med ~  citric.acid + residual.sugar + chlorides + free.sulfur.dioxide + sulphates + alcohol, data=redwine_data,control=rpart.control(minsplit=160), method="class",
parms=list(split='information'))
...
pred2 <- prediction(predict(fit2, type = "prob")[, 2], redwine_data$qual_above_med)
...
White wine: all predictors 
```{r}
fit3  <- rpart( qual_above_med ~ fixed.acidity + volatile.acidity + citric.acid + residual.sugar + chlorides + free.sulfur.dioxide + total.sulfur.dioxide + density + pH + sulphates + alcohol,
                method="class",
                data=whitewine_data, control=rpart.control(minsplit=490),
                parms=list(split='information'))
...
```{r}
pred3 <- prediction(predict(fit, type = "prob")[, 2], whitewine_data$qual_above_med)
...
```{r}
fit4  <- rpart( qual_above_med ~ fixed.acidity + volatile.acidity + citric.acid + residual.sugar + chlorides + free.sulfur.dioxide + sulphates,
method="class",
data=whitewine_data, control=rpart.control(minsplit=490),
parms=list(split='information'))
...
```{r}
pred4 <- prediction(predict(fit, type = "prob")[, 2], whitewine_data$qual_above_med)
